...
# ------------------ Utils: วัน/ช่วง และชื่อชีตรายชื่อ ------------------
WEEKDAY_EN_TO_TH = {
    "Monday": "จันทร์",
    "Tuesday": "อังคาร",
    "Wednesday": "พุธ",
...
def get_session_th_and_cutoff(now_t: time):
    """คืน (session_th, cutoff_time) -> ('เช้า'|'บ่าย', cutoff)"""
...
def extract_student_id(raw_input: str):
    """ดึงเลขล้วน ๆ แล้วตีความตามความยาว (ตามตรรกะเดิม)"""
    digits = re.findall(r"\d+", raw_input or "")
    if not digits:
        return None
    d = "".join(digits)
    if len(d) == 14:      # บางบาร์โค้ด
        return d[3:13]
    if len(d) == 10:      # รหัสนักศึกษา 10 หลัก
        return d
    if 1 <= len(d) <= 3:  # โหมดทดสอบ
        return d
    return None

# ------------------ Google Sheets: client + loaders ------------------
def get_gspread_client_from_secrets(readonly=False):
    sa_info = st.secrets["GOOGLE_SERVICE_ACCOUNT"]
    scopes = [
        "https://www.googleapis.com/auth/spreadsheets.readonly" if readonly else
...
def load_roster_id_and_seat_from_gsheet(sheet_id: str, worksheet_name: str):
    """
    อ่านชุดรายชื่อจาก Google Sheet (worksheet_name เช่น 'จันทร์เช้า')
    header: A='Student ID', B='Full Name', C='Seat'
    คืน: (roster_by_id, roster_by_seat)
    """
    ...
    idx_sid  = idx_of("Student ID")
    idx_name = idx_of("Full Name")
    idx_seat = idx_of("Seat")

    if idx_sid is None or idx_name is None or idx_seat is None:
...
def open_or_create_log_worksheet(sh: gspread.Spreadsheet, name: str) -> gspread.Worksheet:
    """เปิดหรือสร้างชีต log (คนละชีตกับ roster) พร้อม header"""
    try:
...
# ======================== UI & State ========================
st.set_page_config(page_title="QR/Barcode Attendance", page_icon="✅", layout="centered")
...
# เตรียมชีตสำหรับบันทึกผล (เป็นชีตใหม่ คนละแท็บ)
...
st.caption("Tip: On mobile you must use HTTPS (Streamlit Cloud). Allow Camera permission.")
...
def decode_any_barcode(img_bgr) -> str:
    # 1) Try OpenCV BarcodeDetector (1D/2D)
    try:
...
if submitted:
    # อัปเดตช่วง/ชีต roster/log ถ้าข้ามเที่ยงหรือวัน (เมื่อมีการ submit)
    now2 = datetime.now()
    ...
    # ตรวจและบันทึก (ตรรกะเดิม)
    sid = extract_student_id(student_id_input) if student_id_input else None
    seat_norm = normalize_seat(seat_input) if seat_input else None
